...
The sets $\Globals$, $\ThreadLocals$, and $\Locals$ are the names of global, thread-local, and procedure-local variables 
respectively; these sets are mutually disjoint and $\Vars$ denotes their union.
There is an uninterpreted set $\Value$ of values that may be stored in these variables;
$\Store$ is the set of all valuations from $\Vars$
A \civl program is a tuple $(\procs, \actions, \ProcLins, G, \TS)$ with the following components:
\begin{itemize}
\item 
...
The predicate $\rho$ is the precondition of $A$ and the predicate $\alpha$
\item
$\ProcLins$
\item
$\TS$
