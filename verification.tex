...
\actions(\Refines(P')) = (\rho',\alpha',m) \\ \rho' \circ \alpha' \Rightarrow \exists \old(\Local), \Local.\ \alpha
}
...
\actions \vdash s \preceq \ga{e}{\exists \old(\Local), \Local.\ \alpha}
}
...
\forall (\rho,\alpha,m) \in \range(\actions \circ \Refines).\ (\accessVars(\rho) \cap \Local = \emptyset) \wedge (\alpha = ((\exists \old(\Local), \Local.\ \alpha) \wedge \Same(\Local))) \\
\Injective(\Refines) \\
...
\inferrule
{
\procs;\actions;\RefinementAny;\mods \js \FH{e \wedge \locExpr}{s}{e} \\ e \wedge \locExpr \Rightarrow f \geq 0 \\ s \preceq (old(e \wedge \locExpr) \Rightarrow \old(f) > f)
}
