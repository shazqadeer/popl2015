...
\inferrule
{
\actions(\refines(P)) = (\rho,\alpha,m) \\ \actions(\refines(P')) = (\rho',\alpha',m) \\ \rho \Rightarrow \rho' \\ \rho \circ \alpha' \Rightarrow \alpha
}
{P \vdash \call{P'} : A}
\;(\textsc{Call-Action})
$
...
\inferrule
{
\actions(\refines(P)) = (\rho,\alpha,m) \\ \rho \Rightarrow e \\ \jp \FH{\rho}{s}{\alpha}
}
