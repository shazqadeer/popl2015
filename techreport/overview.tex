...
Why is the distinction between preemptive and cooperative semantics so important?
Yield elimination is useful both for the user, since no yield predicates need to be provided, and for \civl,
...
The ability to hide variables and write yield predicates specific to each refinement step in 
an overall proof is an important and novel capability in \civl that facilitate proofs spanning large abstraction gaps.

%% Refinement reasoning requires powerful and
%% flexible mechanisms for reasoning with invariants.  
%% As illustrated in our running example, refinement verification for \exC{TopWB} required 
