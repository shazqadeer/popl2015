...
$\procs$ & $\in$ & $\ProcName \rightarrow (\StateExpr, 2^{\Global \cup \ThreadLocal}, \StateExpr, \Stmt)$ \\
...
The statement $\ablock{e,\lins}{\stmt}$ is a block of statements that may not have a yield staetement inside it;
...
Figure~\ref{fig:syntax} also defines the subset $\mathit{YieldingThread}$ of threads and the subset $\XProgram$ of programs.
...
Similarly, we let $\alpha \circ \rho$ denote the relation obtained by restricting the range of $\alpha$ to $\rho$.
