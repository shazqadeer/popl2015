...
If there exists such an exceptional switch from thread $i$, then it is also the case that the statement at which the switch happened is labeled with $\LM$.
This implies that it is possible to execute thread $i$ via a sequence of left-moving steps either infinitely or until a yield point occurs.
Now, we patch these exceptional switches starting from the earliest one.
To patch the switch from thread $i$, we extend the execution of thread $i$ at the end by a left mover and then commute it earlier by repeated swaps 
