...
The program transformation rules are presented in Figure~\ref{fig:program-transformation}.
...
In the remainder of this section, we will formalize the judgments discussed above in Sections~\ref{sec:linearity},~\ref{sec:refinement}, and~\ref{sec:yield-elimination},
present our soundness theorem in Section~\ref{sec:correctness}, and provide important details of our implementation in Section~\ref{sec:implementation}.
...
For each multiset $V$ over $\Value$, let $\Disjoint(V)$ be true iff the sets $\Set(v)$ for all $v \in V$ are pairwise disjoint.
...
This judgment uses the notation $\Havoc(X)$, where $X \subseteq \Var$, to represent the transition relation 
