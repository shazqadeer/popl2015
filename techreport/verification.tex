...
P \in \dom(\Refines) \\ \actions(\Refines(P)) = (\StateExpr,\Havoc(\{\}),m) 
}
{\Refines;\procs;\actions;\ProcOrNone;\mods;b \jr \FH{\phi}{\call{P}}{\psi};b}
\;(\textsc{Call2})
$
...
P \in \dom(\Refines) \\
\actions(\Refines(P)) = (\StateExpr,\Havoc(\{\}),m)
}
{\Refines;\procs;\actions;\ProcOrNone;\{\};b \jr \FH{\rho \wedge \phi}{\async{P}}{\rho};b}
\;(\textsc{Async2})
$
...
The rule \textsc{Seq} for $\StmtStack;\stmt$ chains the refinmene behavior of $\StmtStack$ with that of $\stmt$.
