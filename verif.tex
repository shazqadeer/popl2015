...
% - Low-level program
% - High-level program
% - When a thread steps, what is that? A step? A state transition? An action?
% - ablock/noyield: Atomic block
% - abstract-atomic procedure
% - atomic action specification (the atomic specification for a procedure)
...
If a program is well-typed and all verification conditions are satisfied, then the program is guaranteed to run without getting stuck (in particular, with no assertion failures). Furthermore, 
...
For every $(l,\yield{e_1}) \in \Yields l \bodies$ and $\ablock{e_2}{s}{r} \in \Ablocks$, we verify that $e$ remains preserved under 
$$\ablock{e_2[\vec{x} \rightarrow \vec{x'}]}{s[\vec{x} \rightarrow \vec{x'}]}{r}$$
...
In this section, we describe the verification conditions generated to verify that $\bodies(P)$ is a correct refinement. Intuitively speaking, we generate verification checks that ensure that every sequence of state transitions performed by $\bodies(P)$ refines an execution of the {\em specification automaton} in Figure~\ref{fig:refChkAut}. 
...
\Ablocks(bs, \procName \ablock{e}{s}{r})   =   \{(\procName, \ablock{e}{s}{r}) \} \\ 
...
For each $(P_{A}, \ablock{e}{s}{r}) \in \Ablocks(\Prog)$, we perform a refinement check as described below. Here 
